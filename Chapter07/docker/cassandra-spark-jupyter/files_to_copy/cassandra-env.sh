...
JVM_OPTS="$JVM_OPTS -XX:+UseG1GC" # Use the Hotspot garbage-first collector.
JVM_OPTS="$JVM_OPTS -XX:MaxGCPauseMillis=500" # Main G1GC tunable: lowering the pause target will lower throughput and vise versa. Have the JVM do less remembered set work during STW, instead 200ms is the JVM default and lowest viable setting 1000ms increases throughput. Keep it smaller than the timeouts in cassandra.yaml.
JVM_OPTS="$JVM_OPTS -XX:G1RSetUpdatingPauseTimePercent=5" # preferring concurrent GC. Reduces p99.9 latency.
JVM_OPTS="$JVM_OPTS -XX:InitiatingHeapOccupancyPercent=25" # Start GC earlier to avoid STW. The default in Hotspot 8u40 is 40%.
...
#JVM_OPTS=" -XX:G1HeapRegionSize=32m"
